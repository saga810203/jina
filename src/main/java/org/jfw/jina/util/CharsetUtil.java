...
public class CharsetUtil {
	...
	public static CharsetEncoder encoder(Charset charset) {
		...
		Map<Charset, CharsetEncoder> map = getEncoderMap();
	...
	public static Map<Charset, CharsetDecoder> getDecoderMap() {
		AbstractAsyncExecutor executor = AbstractAsyncExecutor.ExecutorThread.executor();
		assert executor != null;
		Map<Charset, CharsetDecoder> ret = executor.<Map<Charset, CharsetDecoder>> getObject(CFG_DE_CODER);
		if (ret == null) {
			ret = new HashMap<Charset, CharsetDecoder>();
			((AbstractAsyncExecutor) executor).setObject(CharsetUtil.class, ret);
		}
		return ret;
	}

	public static Map<Charset, CharsetEncoder> getEncoderMap() {
		AbstractAsyncExecutor executor = AbstractAsyncExecutor.ExecutorThread.executor();
		assert executor != null;
		Map<Charset, CharsetEncoder> ret = executor.<Map<Charset, CharsetEncoder>> getObject(CFG_EN_CODER);
		if (ret == null) {
			ret = new HashMap<Charset, CharsetEncoder>();
			((AbstractAsyncExecutor) executor).setObject(CharsetUtil.class, ret);
		}
		return ret;
	}
	...
	public static CharsetDecoder decoder(Charset charset) {
		...
		Map<Charset, CharsetDecoder> map = getDecoderMap();
	...
	private static final Object CFG_EN_CODER = new Object();
	private static final Object CFG_DE_CODER = new Object();
